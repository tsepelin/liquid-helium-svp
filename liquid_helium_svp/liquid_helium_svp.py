...
def density_from_pressure_low_temperature(pressurePa: npt.ArrayLike, temperatureK: npt.ArrayLike) -> npt.ArrayLike:
    ...
    pressurePa, temperatureK = np.broadcast_arrays(
        np.asarray(pressurePa, dtype=float), np.asarray(temperatureK, dtype=float)
    )
    rho_0 = np.asarray(density(temperatureK), dtype=float)

    densities = []
    for p, base_density in zip(pressurePa.ravel(), rho_0.ravel()):
        d = p * -9.86923e-6
        poly = np.array([A3, A2, A1, d])
        roots = np.roots(poly) * 1e3
        real_roots = roots[np.abs(np.imag(roots)) < 1e-5]
        if len(real_roots) == 0:
            raise ValueError("No real density solution for the supplied pressure")
        densities.append(np.real(real_roots[-1]) + base_density)
...
def sound_velocity_first_from_pressure_low_temperature(pressurePa: npt.ArrayLike, temperatureK: npt.ArrayLike) \
    ...
    rho_0 = density(temperatureK) * 1e-3
    velocity = A1 + 2 * A2 * (rho - rho_0) + 3 * A3 * ((rho - rho_0) ** 2)
